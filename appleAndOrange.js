...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
    ...
    let check_apples = apples.map(x=> a + x)
    let check_oranges = oranges.map(x=> b + x)
    
    let total_apples = check_apples.filter(x=> x >= s && x <= t)
    let total_oranges = check_oranges.filter(x=> x >= s && x <= t)
    
    console.log(total_apples.length)
    console.log(total_oranges.length)
